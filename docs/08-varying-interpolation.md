...
```text
 smooth（透视校正）                    flat（不插值）
      v0红                                v0红
      ╱╲                                  ╱╲
     ╱橙╲           颜色沿面渐变          ╱   ╲
    ╱ ★ ╲        ★处=三种权重混合      ╱  ★  ╲      ★处 = 第3个顶点
   ╱_____╲                            ╱_______╲     的纯色（激起顶点）
      v1绿      v2蓝                  整块=蓝色
```

逐步拆解：

1. 顶点着色器里 `smooth out vec3 v_color`（smooth 可省略，是默认值）；
2. 光栅化对每个片元计算重心坐标 (w1,w2,w3)，且**先除以 w 再插值再乘回**——这就是"透视校正"；
3. 片元着色器 `in vec3 v_color` 收到的已经是插好的值；
4. 切 `flat` 后硬件跳过插值，直接取**图元最后一个顶点**的值——所以三角形显示第 3 个顶点的蓝色；
5. App 里旋转三角形（绕 X 轴），三个顶点深度持续变化——smooth 下渐变依然贴合 3D 面，这是"透视校正"的功劳。
