...
## 可视化实验：亲眼看到三种附件（App 第 29 项 / win32 ch29）

本界面把深度附件挂成**深度纹理**（不再是 renderbuffer），于是三种附件都能"看见"：

| 视图 | 看到的内容 | 原理 |
|---|---|---|
| 附件0·颜色 | 正常的彩色画面 | 采样颜色附件纹理 |
| 附件·深度 | 灰度图：越近越亮、越远越暗（画面边缘=最远=纯黑边缘外圈） | FS 直接采样深度纹理的 r 通道，`pow(d,8)` 拉开近处对比 |
| Stencil（win32） | 画面中央出现绿色"洞" | 场景里先画一个小四边形往模板写 1，再用 `glStencilFunc(EQUAL,1)` 画全屏绿色——只有洞里透出绿色 |

**观察要点**：
1. 深度视图里立方体边界清晰、前后关系一目了然——这就是深度测试的"判决数据"；
2. 深度图整体偏白（值都接近 1.0）——正是第 13 章讲的 1/z 非线性：远处精度被压缩；
3. 切换视图时场景不变，变的只是"读哪个附件"——三种附件是同一帧的三份平行记录。
