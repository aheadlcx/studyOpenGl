...
```text
 ...
 TRIANGLES (3,3,3)      STRIP (n-2个)         FAN (共享首点)
  2───┐ ┌──3             1───2              2  3
  │  ╲│╲│              ╱│ ╳ │╲             │╲ │╱
  │   ╲│╲│            ╱ │ ╳ │ ╲            │ ╲│
  0───1 └─4          0──└─3──┘4           0───────
  (0,1,2)(3,4,5)    (0,1,2)(2,1,3)...    (0,1,2)(0,2,3)(0,3,4)

 重启索引:  [stripA的顶点...][0xFFFFFFFF][stripB的顶点...]
            └── 第1条带         └断开        └第2条带（一次draw！）
```

逐步拆解：

1. `glDrawElements` 的第三个参数 mode 决定"顶点怎么绑"：TRIANGLES 每 3 个一组互不相干；STRIP 像"折纸"每新来 1 个顶点就和前 2 个组成新三角形；FAN 全部和第 0 个顶点组队；
2. TRIANGLES 画 10 个三角形要 30 个顶点，STRIP 只要 12 个——省的是顶点处理量；
3. 索引绘制把"用哪些顶点"和"顶点数据"分离：同一批 37 个顶点（圆心+圆周），配 8 种不同索引缓冲就是 8 种图形；
4. 重启索引：遇到 `0xFFFFFFFF` 就"断开重开"，一次调用画两条不相连的带——省掉第二次 draw call 的 CPU 提交开销。
