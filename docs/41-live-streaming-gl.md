...
连麦 = 主播和观众实时互看互听。画面合成有两种分工：

| 方式 | 谁来合成 | 特点 |
|---|---|---|
| **本地混流** | 主播端 GPU（本节） | 省服务器算力；主播端多耗电；观众看到的是主播合成好的一路 |
| **云端混流** | 服务器转码合成 | 终端省电；要付转码成本；多一跳延迟 |
...
- **矩形表驱动布局**：每个画面一个 uv/像素矩形 `{x,y,w,h}`，混画 shader 对落入某矩形的片元用
  `uv = (frag - xy) / wh` 重映射采样那一路纹理。切布局（画中画/左右/上下/宫格）
  **只是换一张矩形表**，不是改代码；
- 工程里等价做法：`glViewport(x,y,w,h)` + `glScissor` 逐路画，天然裁剪到各自矩形；
- **名字标签**：每路一次半透明叠加（`SRC_ALPHA, ONE_MINUS_SRC_ALPHA`），画在混画之后；
- **大小流订阅**：大窗收高分辨率流、小窗收低分辨率流，下行带宽立省一半；
- **分割间隙**：矩形之间留 gap 露出底色，或给小窗描彩边；
- **音频是另一条链**：多路 PCM 混音（叠加 + 限幅 + 时钟对齐）后与混画帧一起打包，与 GL 无关。

> App 第 34 项「节点5 · 连麦本地混画」可以现场切布局：切布局就是换一张矩形表。
